...
- [gpu-solver-v2.md](findings/gpu-solver-v2.md) — **Global factor-once/solve-many: CONFIRMED, ×5.5 over v1** (1.40 s/sample @ B=8, mixed precision, Ra err 4e-5). Phase 0: sizing trivial, ×2.41 RHS dedup, discretization parity with v1 (0.03–0.09%) — the 3–5% "errors" were the mud-column + boundary-truncation conventions; pure-fp32 Schur recursion NaNs (fp64 recursion → fp32 solves is the stable mix). full/ 1000-sample bench (2026-07-19): 1.63 s/sample; ALL large deviations arbitrated — zero genuine v2 errors; the refs carry batch5 artifacts (≤158%), coarse-mesh invasion-torus errors (≤15%, FEM converges *to* v2), and R=40 truncation (≤52% on A8.0).
